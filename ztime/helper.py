...
def trialSequence_multivariate(X_train, X_test, y_train, y_test, window_size = 10, window_size_slope = 5, 
                    ...
                    mask_train = False, mask_test = False):
    ...
    for split_part in zip(X_train_split, X_test_split):
        if varyingLength == True:
            train_dataset = createMultivariateDatabaseVL(split_part[0], window_size = window_size, window_size_slope = window_size_slope, alphabet_size = alphabet_size, alphabet_size_slope= alphabet_size_slope, glob=glob)
            test_dataset = createMultivariateDatabaseVL(split_part[1], window_size = window_size, window_size_slope = window_size_slope, alphabet_size = alphabet_size, alphabet_size_slope = alphabet_size_slope, glob=glob)
    ...
    # ind = np.argpartition(coeff[0], -10)[-10:]
    # for i in ind:
    #     print(i)
    #     print(features[i])
    # ###################################################
